...
import { Filters, Product, Breadcrumb } from '../types';
...
function CatalogPage() {
  ...
  const navigate = useNavigate();
   ...
   const initialFilters: Filters = {
        ...
        price: searchParams.getAll('price').length > 0 ? searchParams.getAll('price').map(Number) : [0, 10000],
  ...
  const [filters, setFilters] = useState<Filters>(initialFilters);
     ...
     const breadcrumbs: Breadcrumb[] = [{ name: 'Каталог товаров', link: '/'}, {name: type.charAt(0).toUpperCase() + type.slice(1), link: `/catalog/${type}` }]


  const filteredProducts = useMemo(() => {
        let filtered = initialProducts.filter(product => product.type.toLowerCase() === type);
    ...
    const typeProducts = useMemo(() => {
       return  initialProducts.filter(product => product.type.toLowerCase() === type);
    }, [type])
         ...
         <div className="flex flex-row ">
            <div className="w-1/4 p-4 bg-gray-100">
                <div className="mb-4">
                  ...
                  <Filter onFilterChange={handleFilterChange} products={typeProducts} initialFilters={filters}/>
            </div>

...
function ProductList({
  ...
  const handleProductClick = () => {
    ...
    const url = `/product?type=${encodeURIComponent(type)}&category=${encodeURIComponent(category)}&brand=${encodeURIComponent(brand)}&model=${encodeURIComponent(model)}`;
    navigate(url);
  };

  return (
    <button onClick={handleProductClick} className='text-left flex flex-col justify-baseline cursor-pointer min-w-[160px] '>
